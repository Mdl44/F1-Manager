...
void MixedCondition::apply_effects(Team* team) {
    if (const auto* t = team) {
        ...
        if (pair1.car) pair1.car->apply_upgrades(mixed_bonus);
        if (pair2.car) pair2.car->apply_upgrades(mixed_bonus);
        ...
        if (pair1.driver) {
            ...
            pair1.driver->apply_upgrades(driver_bonus);
        }
        ...
        if (pair2.driver) {
            ...
            pair2.driver->apply_upgrades(driver_bonus);
        }
    }
}
...
void MixedCondition::remove_effects(Team* team) {
    if (const auto* t = team) {
        ...
        if (pair1.car) pair1.car->apply_downgrades(mixed_bonus);
        if (pair2.car) pair2.car->apply_downgrades(mixed_bonus);

        if (pair1.driver) {
            const int driver_bonus = driver_mixed_bonuses[pair1.driver];
            pair1.driver->apply_downgrades(driver_bonus);
            driver_mixed_bonuses.erase(pair1.driver);
        }

        if (pair2.driver) {
            const int driver_bonus = driver_mixed_bonuses[pair2.driver];
            pair2.driver->apply_downgrades(driver_bonus);
